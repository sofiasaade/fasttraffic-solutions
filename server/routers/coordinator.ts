...
export const coordinatorRouter = router({
  ...
  jobAllFields: adminProcedure
    ...
    .query(async ({ input }) => {
      ...
      const HIDDEN =
        /^(matriz pr|geocode|pdf name|autocad|recap|calculation|lat|lon|id|days to sd|duration|standardized time|category baseline|performance status|impact score|project score|design development|review time|month|year created|timing active|start\s+plan creation|stop time plan creation|plan created|duration plan creation|date of feedback|last status modified|last modified|created time|quality checklist|checklist|permit timing|post permit requested|ind - |total matriz|date last heads up|speed limit|categoria de tiempo|tmp comments|project title|permit extension|plan\/stamp category|location map|calendar info|cesar comments|designers sub-status|single lane|pedestrian impact|cyclist impact|road closure$|speed reduction|combined closures|highway|distance from the storage|acq$|semáforo|city comments files|comment history|designers table|designer name|assignee|collaborators?|dependencies|from field|received status|recieved by|heads up|estado simplificado|posibles|job address match|permit request summary|speed limit|traffic restrictions|project status for design|project status report|phases|file uploaded by client|email sent|date\/time emailed)/i;
      ...
      const projects = jobs
        ...
        .map((j) => {
          ...
          return {
            ...
            company: j.company,
            jobAddress: j.jobAddress,
            projectTitle: j.projectTitle ?? null,
            ...
            municipality: j.municipality ?? null,
            zone: (j as any).zone ?? null,
            requestId: j.requestId ?? null,
            requestorName: j.requestorName ?? null,
            siteContactPhone: j.siteContactPhone ?? null,
            status: j.status,
            subStatus: j.subStatus,
